...
export default function MinhChungPage() {
  useEffect(() => {
        ...
        async function submitData() {
          ...
          if(!compressedBase64) return alert("Vui lòng đính kèm ảnh chụp minh chứng!");
          ...
          try {
            ...
            if(res.success) {
              showResult(true, "✅ Đã gửi minh chứng thành công! Vui lòng chờ xét duyệt.");
            ...
            showResult(false, "❌ Máy chủ không phản hồi. Vui lòng kiểm tra lại kết nối mạng!"); 
          }
        ...
        function prependMinhChung(item) {
          ...
          const image = item.anhUrl ? `<a href="${item.anhUrl}" target="_blank" class="text-blue-600 text-[11px] font-bold hover:underline">🖼 Mở ảnh đính kèm</a>` : '';
          list.insertAdjacentHTML('afterbegin', `<div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100"><span class="float-right px-2 py-0.5 rounded text-[10px] font-bold border bg-yellow-100 text-yellow-800 border-yellow-200">${item.trangThai}</span><h4 class="font-bold text-gray-800 text-sm pr-16">${item.hangMuc}</h4><p class="text-[11px] text-gray-400 mt-1">${item.thoiGian}</p><div class="text-xs text-gray-600 bg-gray-50 p-3 rounded-lg border border-gray-100 mt-3">${item.moTa}</div><div class="mt-3 pt-3 border-t border-gray-50">${image}</div></div>`);
        }

        async function loadHistory() {
          const list = document.getElementById('listMC');
          list.innerHTML = '<p class="text-center text-gray-400 bg-white p-6 rounded-xl shadow-sm text-sm border italic">Đang tải lịch sử...</p>';
          
          try {
            const response = await fetch(API_URL, {
              method: 'POST',
              headers: { 'Content-Type': 'text/plain;charset=utf-8' },
              body: JSON.stringify({ action: 'getMinhChungByTNV', sessionId: sessionId })
            });
            const res = await response.json();
            
            if(res.success) {
              if (res.data.length === 0) {
                list.innerHTML = '<p class="text-center text-gray-400 bg-white p-6 rounded-xl shadow-sm text-sm border italic">Chưa có minh chứng nào được ghi nhận.</p>';
                return;
              }
              ...
              res.data.forEach(item => {
                ...
                if(item.trangThai === 'Đã duyệt') badgeClass = 'bg-green-100 text-green-800 border-green-200';
    ...
    Object.assign(window as any, { logout, showResult, clearImage, previewImage, submitData, loadHistory });
  }, []);
  ...
  <div className="max-w-md mx-auto px-4 pt-20">
    <div className="flex space-x-1 bg-white p-1 rounded-xl shadow-sm border border-gray-100">
      ...
      <a href="/tnv/minhchung" className="flex-1 text-center py-2.5 text-xs font-bold rounded-lg bg-green-700 text-white shadow-sm">Minh chứng</a>
  ...
  <main className="max-w-md mx-auto p-4 mt-2 space-y-6">
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-bold text-gray-600 mb-1">Gửi đến Ban/Khu vực</label>
          <select id="maKhuVucNhan" className="w-full border-2 border-gray-100 rounded-xl p-3 bg-gray-50 focus:outline-none focus:border-green-600 focus:bg-white text-sm font-medium transition">
            <option value="TUDONG">Tự động (Theo khu vực gần nhất)</option>
        ...
        <div>
          <label className="block text-xs font-bold text-gray-600 mb-1">Hạng mục công việc</label>
          <select id="hangMuc" className="w-full border-2 border-gray-100 rounded-xl p-3 bg-gray-50 focus:outline-none focus:border-green-600 focus:bg-white text-sm font-medium transition">
        ...
        <div>
          ...
          <textarea id="moTa" rows="3" className="w-full border-2 border-gray-100 rounded-xl p-3 bg-gray-50 focus:outline-none focus:border-green-600 focus:bg-white text-sm transition" placeholder="Ghi rõ bạn đã làm việc gì, ở đâu, từ mấy giờ đến mấy giờ..."></textarea>
        </div>
        
        <div>
          <label className="block text-xs font-bold text-gray-600 mb-2">Ảnh chụp minh chứng</label>
          <input type="file" id="anhUpload" accept="image/*" onChange={(event) => { (window as any).previewImage(event); }} className="w-full text-sm text-gray-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 cursor-pointer transition" />
          
          <div id="imagePreviewContainer" className="hidden mt-3 relative">
            <img id="imagePreview" className="w-full h-auto rounded-xl border border-gray-200 shadow-sm" src={undefined} alt="Preview" />
            <button onClick={() => { (window as any).clearImage(); }} className="absolute top-2 right-2 bg-white text-red-500 rounded-full w-8 h-8 flex items-center justify-center shadow-md font-bold text-lg hover:bg-red-50">✕</button>
          </div>
        </div>
        
        <button id="btnSubmit" onClick={() => { (window as any).submitData(); }} className="w-full bg-green-700 text-white font-bold py-4 rounded-xl mt-2 shadow-lg hover:bg-green-800 active:scale-95 transition-all">
    ...
    <div>
      <div className="flex justify-between items-end mb-3 px-1">
        ...
        <button onClick={() => { (window as any).loadHistory(); }} className="text-[11px] text-blue-600 font-bold hover:underline uppercase tracking-tighter">Làm mới</button>
      </div>
      <div id="listMC" className="space-y-3">
        <p className="text-center text-gray-400 bg-white p-6 rounded-xl shadow-sm border italic text-sm">Đang tải lịch sử...</p>
      </div>
    </div>
