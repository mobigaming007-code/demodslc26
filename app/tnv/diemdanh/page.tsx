...
export default function DiemDanhPage() {
  useEffect(() => {
        ...
        async function initCameraScanner() {
          try {
            if (!navigator.mediaDevices?.getUserMedia) throw new Error('Camera không được hỗ trợ');
            const [qr, zxing] = await Promise.all([import('html5-qrcode'), import('@zxing/browser')]);
            window.Html5Qrcode = qr.Html5Qrcode;
            window.ZXingBrowser = zxing;
            await startCameraScanner();
          } catch (error) {
            showResult('❌ Không thể khởi tạo camera. Hãy cấp quyền Camera và thử lại.', 'error');
          }
        }
        ...
        async function startCameraScanner() {
          ...
          const onScan = (decodedText) => { try { html5QrCode.pause(true); } catch (e) {} handleDiemDanh(decodedText); };
          try {
            ...
            try {
              ...
              console.error('Không mở được camera', rearError, frontError);
              try { await html5QrCode.clear(); } catch (e) {}
              html5QrCode = null;
              cameraStarted = false;
              showResult('❌ Không thể mở Camera. Bạn vẫn có thể tải ảnh QR bên dưới.', 'error');
            }
          }
        }
