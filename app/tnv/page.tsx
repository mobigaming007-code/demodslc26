...
export default function TnvPage() {
  useEffect(() => {
...

    {
      ...
      if (currentSession) {
        window.location.href =
          currentRole === "Admin" ? "/admin" : "/tnv/diemdanh";
      }
    }
    ...
    async function handleLogin(role) {
      const btn =
        role === "TNV"
          ? document.getElementById("btnTNV")
      ...
      if (role === "TNV") {
        ...
        if (!payload.maTNV)
          return showMsg(false, "Vui lòng nhập Mã TNV hoặc Số điện thoại!");
      } else {
        ...
        if (!payload.user || !payload.pass)
          return showMsg(false, "Vui lòng nhập đầy đủ Email và Mật khẩu!");
      }
      ...
      try {
        ...
        showMsg(
          false,
          "❌ Máy chủ không phản hồi. Vui lòng kiểm tra lại quyền truy cập (Anyone) trên Google Script!",
        );
        btn.disabled = false;
        btn.innerText = role === "TNV" ? "VÀO HỆ THỐNG" : "XÁC THỰC QUẢN TRỊ";
      }
      ...
      <main className="bg-gray-100 h-screen flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-3xl shadow-xl max-w-sm w-full border border-gray-100">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-black text-green-700 leading-tight">
              CHƯƠNG TRÌNH
            </h1>
            ...
            <p className="text-xs text-gray-400 mt-2 font-medium tracking-widest uppercase">
              Cổng thông tin quản lý hoạt động của Tình nguyện viên
            </p>
          </div>

          <div
            id="resultBox"
            className="hidden p-3 rounded-xl text-sm text-center font-bold mb-6 transition-all"
          ></div>

          <div id="formTNV" className="space-y-5">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-2 ml-1">
                Tài khoản Tình nguyện viên
              </label>
              <input
                type="text"
                id="maTNV"
                onKeyDown={(event) => {
                  if (event.key === "Enter") {
                    ...
                    (window as any).handleLogin("TNV");
                  }
                }}
                className="w-full border-2 border-gray-100 rounded-2xl p-4 bg-gray-50 focus:outline-none focus:border-green-600 focus:bg-white transition-all uppercase font-bold text-gray-700"
                placeholder="Mã TNV hoặc Số điện thoại..."
              />
            </div>
            <button
              id="btnTNV"
              ...
              className="w-full bg-green-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-green-200 hover:bg-green-800 active:scale-95 transition-all"
            >
              VÀO HỆ THỐNG
            </button>
            <button
              ...
              className="w-full text-xs text-gray-400 underline hover:text-blue-900 transition"
            >
              Dành cho Ban điều hành
            </button>
          </div>

          <div id="formAdmin" className="hidden space-y-5">
            <div>
              <label className="block text-xs font-bold text-blue-900 uppercase mb-2 ml-1">
                Email Ban điều hành
              </label>
              <input
                type="email"
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter") {
                    event.preventDefault();
                    (window as any).handleLogin("Admin");
                  }
                }}
                className="w-full border-2 border-gray-100 rounded-2xl p-4 bg-gray-50 focus:outline-none focus:border-blue-900 focus:bg-white transition-all font-bold mb-4"
              ...
              <input
                ...
                className="w-full border-2 border-gray-100 rounded-2xl p-4 bg-gray-50 focus:outline-none focus:border-blue-900 focus:bg-white transition-all font-bold"
                placeholder="••••••••"
              />
            </div>
            <button
              ...
              className="w-full bg-blue-900 text-white font-bold py-4 rounded-2xl shadow-lg shadow-blue-200 hover:bg-black active:scale-95 transition-all"
            >
              XÁC THỰC QUẢN TRỊ
            </button>
            <button
              ...
              className="w-full text-xs text-gray-400 underline hover:text-green-700 transition"
            >
              Quay lại trang Tình nguyện viên
            </button>
          </div>
        </div>
      </main>
