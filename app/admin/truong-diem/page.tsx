...
export default function truongDiemPage() {
  useEffect(() => {
        ...
        { loadDiemTrucOptions(); loadTruongDiem(); };
    ...
    async function openEditGCN(maYC, currentStatus, currentNote) {
      ...
      try {
        ...
        async function submitTruongDiem() {
          ...
          const res = await response.json();
          if (res.success) { alert('Đã thêm trưởng điểm trực'); } else alert(res.error);
        }
    ...
    Thao tác này sẽ không công nhận quyền lợi điểm danh của Trưởng điểm trực.`)) return;
          ...
          if (res.success) {
            ...
            const button = document.querySelector(`button[onclick*="${rowNumber}"]`);
            const card = button && button.closest('.item-row');
            if (card) card.remove();
