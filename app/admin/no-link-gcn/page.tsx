...
export default function noLinkGcnPage() {
  useEffect(() => {
        ...
        async function submitUpdateLink(maYC) {
          ...
          const res = await response.json();
          if (res.success) {
            const button = document.querySelector(`button[onclick*="${maYC}"]`);
            const card = button && button.closest('.border');
            if (card) card.remove();
