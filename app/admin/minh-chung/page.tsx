...
export default function minhChungPage() {
  useEffect(() => {
        ...
        async function submitDuyetMC(maMC, status) {
          ...
          const res = await response.json();
          if (res.success) {
            const button = document.querySelector(`button[onclick*="${maMC}"]`);
            const card = button && button.closest('.border.rounded-xl');
            if (card) card.remove();
            alert("Xong!");
          }
        }
