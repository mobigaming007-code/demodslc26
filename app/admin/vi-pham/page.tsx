...
export default function viPhamPage() {
  ...
  return <main className="min-h-screen bg-gray-100 pb-10"><nav className="bg-gray-900 text-white p-4 shadow-md sticky top-0 z-50 flex flex-wrap gap-3 items-center"><Link href="/admin/tong-quan" className="font-bold text-yellow-400 mr-auto">ADMIN DSLC</Link><span id="adminNameDisplay" className="text-xs bg-gray-700 px-2 py-1 rounded"></span><button onClick={() => (window as any).logout()} className="text-xs bg-red-600 hover:bg-red-700 px-3 py-1.5 rounded-lg font-bold">Đăng xuất</button></nav><div className="max-w-5xl mx-auto px-2 pt-4 mb-4"><div className="menu-scroll bg-white p-1 rounded-xl shadow-sm border border-gray-200"><div className="flex space-x-1 min-w-max"><Link href="/admin/tong-quan" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Tổng quan</Link><Link href="/admin/minh-chung" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Duyệt minh chứng</Link><Link href="/admin/gcn" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Duyệt GCN</Link><Link href="/admin/no-link-gcn" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Nợ link GCN</Link><Link href="/admin/lich-su-minh-chung" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Lịch sử minh chứng</Link><Link href="/admin/lich-su-gcn" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Lịch sử GCN</Link><Link href="/admin/truong-diem" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Trưởng điểm</Link><Link href="/admin/vi-pham" className="px-4 py-2 text-xs font-bold rounded-lg transition bg-gray-900 text-white">Ghi nhận sai phạm</Link><Link href="/admin/cap-tai-khoan" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Cấp tài khoản</Link></div></div></div><section className="max-w-5xl mx-auto px-2"><div id="tab-vipham" className="tab-content active">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-red-50 px-4 py-3 border-b border-red-100 flex justify-between">
          <div>
            <h2 className="font-bold text-red-800 text-sm">Ghi nhận / Duyệt sai phạm TNV</h2>
            <p className="text-[11px] text-red-600 mt-1">
              Admin khu vực chỉ gửi báo cáo. Admin/SuperAdmin duyệt để trừ buổi.
            </p>
          </div>
          <button onClick={() => { (window as any).loadViPham(); }} className="text-xs text-blue-600 underline">Làm mới</button>
        </div>

        <div className="p-4 space-y-5 text-sm">
          <div className="bg-gray-50 border rounded-xl p-3">
            <h3 className="font-bold text-gray-800 text-xs uppercase mb-3">1. Tra cứu TNV và ghi nhận sai phạm</h3>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              <input id="vp-ma" className="border p-2 rounded text-xs" placeholder="Nhập mã TNV hoặc SĐT" />
              <button onClick={() => { (window as any).traCuuViPham(); }} className="bg-gray-900 text-white font-bold rounded text-xs px-3 py-2">Tra cứu</button>
              <input id="vp-khuvuc" className="border p-2 rounded text-xs md:col-span-2" placeholder="Mã khu vực, ví dụ D1 / KV_MEDIA. Bỏ trống sẽ lấy phạm vi đầu tiên của admin khu vực" />
            </div>
            <div id="vp-info" className="hidden bg-white border rounded-xl p-3 mt-3"></div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3">
              <textarea id="vp-noidung" className="border p-2 rounded text-xs md:col-span-2" rows="3" placeholder="Nội dung vi phạm"></textarea>
              <input id="vp-minhchung" className="border p-2 rounded text-xs md:col-span-2" placeholder="Link minh chứng báo cáo sai phạm (ảnh/video/Drive nếu có)" />
              <input id="vp-quyetdinh" className="border p-2 rounded text-xs" placeholder="Quyết định xử phạt (Admin/SuperAdmin bắt buộc khi xử phạt trực tiếp)" />
              <input id="vp-link" className="border p-2 rounded text-xs" placeholder="Link thông báo / quyết định" />
              <input id="vp-tru" type="number" step="0.5" min="0" className="border p-2 rounded text-xs" placeholder="Số buổi trừ / đề xuất trừ" />
              <button onClick={() => { (window as any).submitViPham(); }} className="bg-red-700 text-white font-bold rounded text-xs px-3 py-2">
                Gửi ghi nhận / báo cáo sai phạm
              </button>
            </div>
            <p className="text-[11px] text-gray-500 mt-2 leading-relaxed">
              Nếu là Admin khu vực: hệ thống chỉ tạo báo cáo chờ duyệt. Nếu là Admin/SuperAdmin: hệ thống ghi quyết định vào BaoCaoViPham và tự động ghi sang ViPham.
            </p>
          </div>

          <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-3">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-bold text-yellow-800 text-xs uppercase">2. Báo cáo sai phạm / Chờ duyệt / Thực thi</h3>
              <button onClick={() => { (window as any).loadBaoCaoViPham(); }} className="text-xs text-blue-600 underline">Tải báo cáo</button>
            </div>
            <div id="listBaoCaoViPham" className="space-y-3"></div>
          </div>

          <div className="bg-white border rounded-xl p-3">
            <div className="flex justify-between items-center mb-3">
              <h3 className="font-bold text-gray-800 text-xs uppercase">3. Vi phạm chính thức đang tính trừ buổi</h3>
              <button onClick={() => { (window as any).loadLichSuViPhamChinhThuc(); }} className="text-xs text-blue-600 underline">Tải lịch sử chính thức</button>
            </div>
            <div id="listViPham" className="space-y-2"></div>
          </div>
        </div>
      </div>
    </div></section></main>;
