...
export default function tongQuanPage() {
  useEffect(() => {
        ...
        async function loadDashboard() {
          try {
            ...
            if (res.success) {
              // The request can resolve after Next.js has unmounted this route.
              // Update only dashboard nodes that are still present in the DOM.
              const setStat = (id, value) => {
                const element = document.getElementById(id);
                if (element) element.textContent = String(value ?? 0);
              };
              setStat('stat-mc', res.stats.mcChoDuyet);
              setStat('stat-gcn', res.stats.gcnChoDuyet);
              setStat('stat-nolink', res.stats.noLinkGCN);
              setStat('stat-raw', res.stats.tongLuotRaw);
  ...
  return <main className="min-h-screen bg-gray-100 pb-10"><nav className="bg-gray-900 text-white p-4 shadow-md sticky top-0 z-50 flex flex-wrap gap-3 items-center"><Link href="/admin/tong-quan" className="font-bold text-yellow-400 mr-auto">ADMIN DSLC</Link><span id="adminNameDisplay" className="text-xs bg-gray-700 px-2 py-1 rounded"></span><button onClick={() => (window as any).logout()} className="text-xs bg-red-600 hover:bg-red-700 px-3 py-1.5 rounded-lg font-bold">Đăng xuất</button></nav><div className="max-w-5xl mx-auto px-2 pt-4 mb-4"><div className="menu-scroll bg-white p-1 rounded-xl shadow-sm border border-gray-200"><div className="flex space-x-1 min-w-max"><Link href="/admin/tong-quan" className="px-4 py-2 text-xs font-bold rounded-lg transition bg-gray-900 text-white">Tổng quan</Link><Link href="/admin/minh-chung" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Duyệt minh chứng</Link><Link href="/admin/gcn" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Duyệt GCN</Link><Link href="/admin/no-link-gcn" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Nợ link GCN</Link><Link href="/admin/lich-su-minh-chung" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Lịch sử minh chứng</Link><Link href="/admin/lich-su-gcn" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Lịch sử GCN</Link><Link href="/admin/truong-diem" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Trưởng điểm</Link><Link href="/admin/vi-pham" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Ghi nhận sai phạm</Link><Link href="/admin/cap-tai-khoan" className="px-4 py-2 text-xs font-bold rounded-lg transition text-gray-600 hover:bg-gray-100">Cấp tài khoản</Link></div></div></div><section className="max-w-5xl mx-auto px-2"><div id="tab-dashboard" className="tab-content active active">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center">
          <p className="text-[10px] text-gray-500 font-bold uppercase">MC Chờ Duyệt</p>
          <p id="stat-mc" className="text-2xl font-bold text-blue-600 mt-1">-</p>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center">
          <p className="text-[10px] text-gray-500 font-bold uppercase">GCN Chờ Cấp</p>
          <p id="stat-gcn" className="text-2xl font-bold text-yellow-600 mt-1">-</p>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center">
          <p className="text-[10px] text-gray-500 font-bold uppercase">GCN Nợ Link</p>
          <p id="stat-nolink" className="text-2xl font-bold text-orange-600 mt-1">-</p>
        </div>
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 text-center">
          <p className="text-[10px] text-gray-500 font-bold uppercase">Tổng lượt quét</p>
          <p id="stat-raw" className="text-2xl font-bold text-green-600 mt-1">-</p>
        </div>
      </div>
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-gray-50 px-4 py-3 border-b flex justify-between">
          <h2 className="font-bold text-gray-800 text-sm">Điểm danh gần nhất (Toàn hệ thống)</h2>
          <button onClick={() => { (window as any).loadDashboard(); }} className="text-xs text-blue-600 underline">Làm mới</button>
        </div>
        <div id="recentLogs" className="p-4 space-y-2"></div>
      </div>
    </div></section></main>;
}
